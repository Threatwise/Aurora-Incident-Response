...
Aurora now lets you tag timeline activity with MITRE ATT&CK techniques and export incident context straight to PDF briefings or STIX 2.1 bundles for sharing with partners.

Lateral Movement
![Lateral movement visualization](./images/lateral.png "Lateral Movement Visualization")

Visual Timeline
![Timeline visualization](./images/timeline.png "Visual Timeline")
...
[![Aurora walkthrough video thumbnail](http://img.youtube.com/vi/2j2XYcqQIm0/0.jpg)](http://www.youtube.com/watch?v=2j2XYcqQIm0 "Aurora Incident Response Walkthrough")
...
With `nodejs` installed, checkout the [Aurora GitHub repository](https://github.com/cyb3rfox/Aurora-Incident-Response) (or fork first if you want to contribute).
...
#### Security-related environment option

By default, Aurora will block invalid TLS certificates inside the application window. If you need to work with lab systems that use self-signed certificates during development, you can temporarily allow them by setting the following environment variable before starting the app:

Linux/macOS:

```bash
AURORA_ALLOW_INSECURE_CERTS=1 npm start
```

Windows PowerShell:

```powershell
$env:AURORA_ALLOW_INSECURE_CERTS=1; npm start
```

Only use this for development or trusted lab environments.
...
Handles CSV, PDF, and STIX exports

Notes:
- CSV exports are quoted and hardened to mitigate spreadsheet formula injection by prefixing dangerous leading characters.
- Timeline views can be exported directly to PDF reports or STIX 2.1 bundles, including MITRE ATT&CK annotations.
