...
function registerComponents() {
    ...
    try {
            ...
            try {
                ...
                switch (event.target) {
                    ...
                    case 'help:online_help':
                        ...
                        break;
                    default:
                        logger.warn('Unknown toolbar event target', { target: event.target });
                }
            } catch (error) {
                logger.error('Error handling toolbar click', { target: event.target, error: error.message });
            }
        };
        ...
        logger.error('Failed to register GUI components', { error: error.message, stack: error.stack });
        throw error;
    }
    ...
    w2ui.sidebar.onClick = (event) => {
        try {
            ...
            const sidebarHandlers = {
                ...
                'timeline': () => {
                    ...
                    if (lockedByMe) {
                        ...
                        const pyramidOptions = case_data.pyramid_levels ? case_data.pyramid_levels.map(level => level.text) : [];
                        w2ui.grd_timeline.getColumn('pyramid_pain').editable.items = pyramidOptions;
                    }
                ...
                'email': () => {
                    syncAllChanges();
                    if (lockedByMe) {
                        w2ui.grd_email.getColumn('hostname').editable.items = case_data.systems;
                    }
                ...
                'files': () => {
                    syncAllChanges();
                    if (lockedByMe) {
                        w2ui.grd_files.getColumn('hostname').editable.items = case_data.systems;
                    }
                ...
                'processes': () => {
                    syncAllChanges();
                    if (lockedByMe) {
                        w2ui.grd_processes.getColumn('hostname').editable.items = case_data.systems;
                    }
                ...
                'web_activity': () => {
                    syncAllChanges();
                    if (lockedByMe) {
                        w2ui.grd_web_activity.getColumn('hostname').editable.items = case_data.systems;
                    }
                ...
                'persistence': () => {
                    syncAllChanges();
                    if (lockedByMe) {
                        w2ui.grd_persistence.getColumn('hostname').editable.items = case_data.systems;
                        w2ui.grd_persistence.getColumn('mitre_technique').editable.items = case_data.attack_techniques;
                    }
            ...
            logger.error('Error handling sidebar click', { target: event.target, error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'killchain_builder':
                    ...
                    break;
                default:
                    logger.warn('Unknown timeline toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling timeline toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_timeline.onMenuClick = (event) => {
        try {
            if (event.menuItem.id === 'duplicate') {
                ...
                const event_source_host = w2ui.grd_timeline.get(event.recid).event_source_host
                ...
                logger.warn('Unknown timeline menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling timeline menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_investigated_systems.toolbar.onClick = (event) => {
        try {
            ...
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid) , date_added: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown investigated systems toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling investigated systems toolbar click', { error: error.message });
        }
    };

    w2ui.grd_investigated_systems.onChange = (event) => {
        try {
            w2ui.grd_investigated_systems.records[event.index].date_updated = Date.now()
        } catch (error) {
            logger.error('Error handling investigated systems change', { error: error.message });
        }
    };
    ...
    w2ui.grd_investigated_systems.onMenuClick = (event) => {
        try {
            if (event.menuItem.id === 'to_tl') {
                ...
                let first_compromise = w2ui.grd_investigated_systems.get(event.recid).first_compromise
                ...
                let summary = w2ui.grd_investigated_systems.get(event.recid).summary
                let analysis_required = w2ui.grd_investigated_systems.get(event.recid).analysis_required
                ...
                logger.warn('Unknown investigated systems menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling investigated systems menu click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown malware toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling malware toolbar click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.menuItem.id) {
                ...
                case 'vt':
                    ...
                    break;
                default:
                    logger.warn('Unknown malware menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling malware menu click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown accounts toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling accounts toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_accounts.onMenuClick = (event) => {
        try {
            if (event.menuItem.id === 'duplicate') {
                ...
                logger.warn('Unknown accounts menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling accounts menu click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown network toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling network toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_network.onMenuClick = (event) => {
        try {
            switch(event.menuItem.id) {
                ...
                default:
                    logger.warn('Unknown network menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling network menu click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown exfiltration toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling exfiltration toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'duplicate': {
                    ...
                    const stagingsystem = w2ui.grd_exfiltration.get(event.recid).stagingsystem
                ...
                default:
                    logger.warn('Unknown exfiltration menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling exfiltration menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_osint.toolbar.onClick = (event) => {
        try {
            ...
            logger.error('Error handling OSINT toolbar click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown systems toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling systems toolbar click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown investigators toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling investigators toolbar click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown evidence toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling evidence toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_evidence.onMenuClick = (event) => {
        try {
            if (event.menuItem.id === 'duplicate') {
                ...
                logger.warn('Unknown evidence menu event', { menuId: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling evidence menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_actions.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), date_added: Date.now(),date_due: Date.now()} );
                    break;
            ...
            logger.error('Error handling actions toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown case notes toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling case notes toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_email.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), date_received: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
            ...
            logger.error('Error handling email toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'to_tl':
                    ...
                    break;
                default:
                    logger.warn('Unknown email menu event', { menuItem: event.menuItem.id });
            }
        } catch (error) {
    ...
    w2ui.grd_files.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), date_found: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
            ...
            logger.error('Error handling files toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'vt':
                    ...
                    break;
                default:
                    logger.warn('Unknown files menu event', { menuItem: event.menuItem.id });
            }
        } catch (error) {
    ...
    w2ui.grd_processes.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), execution_time: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown processes toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling processes toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'to_tl':
                    ...
                    break;
                default:
                    logger.warn('Unknown processes menu event', { menuItem: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling processes menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_web_activity.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), date_observed: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown web activity toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling web activity toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'to_network':
                    ...
                    break;
                default:
                    logger.warn('Unknown web activity menu event', { menuItem: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling web activity menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_persistence.toolbar.onClick = (event) => {
        try {
            switch(event.target) {
                case 'add':
                    currentgrid.add({ recid: getNextRECID(currentgrid), date_discovered: Date.now()});
                    break;
                case 'remove':
                    currentgrid.remove(currentgrid.getSelection())
                    break;
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown persistence toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling persistence toolbar click', { error: error.message });
        }
    };
        ...
        try {
            switch (event.menuItem.id) {
                ...
                case 'to_tl':
                    ...
                    break;
                default:
                    logger.warn('Unknown persistence menu event', { menuItem: event.menuItem.id });
            }
        } catch (error) {
            logger.error('Error handling persistence menu click', { error: error.message });
        }
    };
    ...
    w2ui.grd_threat_intel.toolbar.onClick = (event) => {
        try {
            ...
            if (event.target === 'add') {
                ...
                logger.warn('Unknown threat intel toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling threat intel toolbar click', { error: error.message });
        }
    };
        ...
        try {
            ...
            switch(event.target) {
                ...
                case 'export':
                    ...
                    break;
                default:
                    logger.warn('Unknown campaigns toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling campaigns toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_add_misp.toolbar.onClick = (event) => {
        try {
            ...
            logger.error('Error handling MISP toolbar click', { error: error.message });
        }
    };
    ...
    w2ui.grd_import_mapping.toolbar.onClick = (event) => {
        try {
            if (event.target === 'import') {
                ...
                logger.warn('Unknown import mapping toolbar event', { target: event.target });
            }
        } catch (error) {
            logger.error('Error handling import mapping toolbar click', { error: error.message });
        }
    };
